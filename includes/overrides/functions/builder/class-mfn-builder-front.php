  	...
  	public function show_wraps($wrap, $w, $vb, $s_iterate = false) {
  		...
  		// Muffin Builder ACM compatibility
			...
			if ( ! empty( $wrap['attr']['animate'] ) ) {
				$wrap_class[] = 'animate';
				$wrap_class[] = 'gsap-animate';
				$animate = 'data-anim-type="'. esc_attr($wrap['attr']['animate']) .'"';
				$animate .= ' data-animation-type="'. esc_attr($wrap['attr']['animate']) .'"';

				$animation_speed = !empty($wrap['attr']['animation_speed']) ? $wrap['attr']['animation_speed'] : '';
				if ( $animation_speed !== '' ) {
					$animate .= ' data-duration="'. esc_attr($animation_speed) .'"';
				}

				$animation_delay = !empty($wrap['attr']['css_advanced_animation_delay']) ? $wrap['attr']['css_advanced_animation_delay'] : '';
				if ( $animation_delay !== '' ) {
					$animate .= ' data-delay="'. esc_attr($animation_delay) .'"';
				}

				$split_text_type = !empty($wrap['attr']['split_text_type']) ? $wrap['attr']['split_text_type'] : '';
				if ( $split_text_type !== '' ) {
				...
				$split_text_stagger = !empty($wrap['attr']['split_text_stagger']) ? $wrap['attr']['split_text_stagger'] : '';
				if ( $split_text_stagger !== '' ) {
					$animate .= ' data-stagger="'. esc_attr($split_text_stagger) .'"';
				}
			}

			if ( isset( $wrap['attr']['scroll_smoother_enable'] ) && (string) $wrap['attr']['scroll_smoother_enable'] === '1' ) {
				$wrap_class[] = 'gsap-smoother-fx';
				$scroll_smoother_speed = ! empty( $wrap['attr']['scroll_smoother_speed'] ) ? $wrap['attr']['scroll_smoother_speed'] : '1';
				$scroll_smoother_lag = ( isset( $wrap['attr']['scroll_smoother_lag'] ) && $wrap['attr']['scroll_smoother_lag'] !== '' )
					? $wrap['attr']['scroll_smoother_lag']
					: '0';
				$animate .= ' data-speed="'. esc_attr( (string) $scroll_smoother_speed ) .'"';
				$animate .= ' data-lag="'. esc_attr( (string) $scroll_smoother_lag ) .'"';
			}
    	...
    	if( !empty($item['type']) && $item['type'] == 'shop_products' && function_exists('is_woocommerce') ) $item_class[] = 'woocommerce';
        	...
        	return;
				...
				if ( ! empty( $item['attr']['animate'] ) ) {
					$item_class[] = 'animate';
					$item_class[] = 'gsap-animate';
					$animate = 'data-anim-type="'. esc_attr($item['attr']['animate']) .'"';
					$animate .= ' data-animation-type="'. esc_attr($item['attr']['animate']) .'"';

					$animation_speed = !empty($item['attr']['animation_speed']) ? $item['attr']['animation_speed'] : '';
					if ( $animation_speed !== '' ) {
						$animate .= ' data-duration="'. esc_attr($animation_speed) .'"';
					}

					$animation_delay = !empty($item['attr']['css_advanced_animation_delay']) ? $item['attr']['css_advanced_animation_delay'] : '';
					if ( $animation_delay !== '' ) {
						$animate .= ' data-delay="'. esc_attr($animation_delay) .'"';
					}

					$split_text_type = !empty($item['attr']['split_text_type']) ? $item['attr']['split_text_type'] : '';
					if ( $split_text_type !== '' ) {
						$animate .= ' data-split-type="'. esc_attr($split_text_type) .'"';
					}
					...
					$split_text_stagger = !empty($item['attr']['split_text_stagger']) ? $item['attr']['split_text_stagger'] : '';
					if ( $split_text_stagger !== '' ) {
						$animate .= ' data-stagger="'. esc_attr($split_text_stagger) .'"';
					}
				}

				if ( isset( $item['attr']['scroll_smoother_enable'] ) && (string) $item['attr']['scroll_smoother_enable'] === '1' ) {
					$item_class[] = 'gsap-smoother-fx';
					$scroll_smoother_speed = ! empty( $item['attr']['scroll_smoother_speed'] ) ? $item['attr']['scroll_smoother_speed'] : '1';
					$scroll_smoother_lag = ( isset( $item['attr']['scroll_smoother_lag'] ) && $item['attr']['scroll_smoother_lag'] !== '' )
						? $item['attr']['scroll_smoother_lag']
						: '0';
					$animate .= ' data-speed="'. esc_attr( (string) $scroll_smoother_speed ) .'"';
					$animate .= ' data-lag="'. esc_attr( (string) $scroll_smoother_lag ) .'"';
				}

				if ( !empty( $item['attr']['image_parallax_enable'] ) && $item['attr']['image_parallax_enable'] == '1' ) {
					$item_class[] = 'gsap-scroll-linked';
					$image_parallax_speed = !empty($item['attr']['image_parallax_speed']) ? $item['attr']['image_parallax_speed'] : '1';
					$image_parallax_lag = !empty($item['attr']['image_parallax_lag']) ? $item['attr']['image_parallax_lag'] : '0';
					$animate .= ' data-speed="'. esc_attr($image_parallax_speed) .'"';
					$animate .= ' data-lag="'. esc_attr($image_parallax_lag) .'"';
					$animate .= ' data-scroll-animation="parallaxY"';
				}
